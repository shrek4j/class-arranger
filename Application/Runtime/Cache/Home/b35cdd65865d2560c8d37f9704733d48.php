...
<div class="main-view">
    ...
    <div class="main-view-inner">
        <div class="main-view-inner-bar">
            <span style="cursor:pointer" data-toggle="modal" data-target="#addClassTypeModal">
                <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>&nbsp;新建课程分类
            </span>
        </div>
                
        <table class="main-view-inner-table table table-bordered table-striped table-hover text-center">
            <thead><tr class="title"><td>编号</td><td>课程分类</td><td>操作</td></tr></thead>
            <tbody>
              <?php if(is_array($classtypeList)): foreach($classtypeList as $key=>$vo): ?><tr>
                  <td><?php echo ($num++); ?></td>
                  <td><?php echo ($vo["name"]); ?></td>
                    <td>
                        <span style="cursor:pointer" class="glyphicon glyphicon-trash cell-operation-gutter" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="删除" onclick="deleteClassType(<?php echo ($vo["id"]); ?>,'<?php echo ($vo["name"]); ?>')"></span>
                    </td>
        ...
        <nav class="text-center">
            <ul class="pagination">
                <li><a href="showClassTypes?pageNo=1" aria-label="Begin"><span aria-hidden="true">首页</span></a></li>
                <li <?php if($pageNo == 1): ?>class="disabled"<?php else: endif; ?>>
                  <a <?php if($pageNo == 1): else: ?>href="showClassTypes?pageNo=<?php echo ($pageNo-1); ?>"<?php endif; ?> aria-label="Previous">
                    <span aria-hidden="true">上一页</span>
                  </a>
                </li>
                <li class="active"><a href="#"><?php echo ($pageNo); ?></a></li>
                <li <?php if($pageNo*$pageSize < $total): else: ?>class="disabled"<?php endif; ?>>
                  <a <?php if($pageNo*$pageSize < $total): ?>href="showClassTypes?pageNo=<?php echo ($pageNo+1); ?>"<?php else: endif; ?> aria-label="Next">
                    <span aria-hidden="true">下一页</span>
                  </a>
                </li>
                <li><a href="showClassTypes?pageNo=<?php echo ($howMangPages); ?>" aria-label="End"><span aria-hidden="true">尾页</span></a></li>
            </ul>
        </nav>
    </div>
</div>
...
<!-- Modal -->
<div class="modal fade" id="addClassTypeModal" tabindex="1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          <div class="container-fluid">
              <div class="row">
                  <div class="col-md-2">
                    <label for="classtype" class="control-label">课程分类名称:</label>
                  </div>
                  <div class="col-md-6">
                    <input type="text" class="form-control" id="classtype"/>
                  </div>
              </div>
          </div>
      </div>
      <div class="modal-footer">
          <button type="button" class="btn btn-primary add-classtype">保存</button>
